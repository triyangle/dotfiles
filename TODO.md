...
- **Theme / color scheme on fresh machines.** Claude Code persists theme in `~/.claude.json` (not `~/.claude/settings.json`), which is user-local and not meant to be synced per the docs. First-run wizard prompts for it when that file is missing. Three options: (1) click through once per machine, (2) add a manual one-liner to run after bootstrap: `jq '.theme = "dark-daltonized" | .hasCompletedOnboarding = true' ~/.claude.json > /tmp/cj && mv /tmp/cj ~/.claude.json` (also works on a pre-seeded empty object), (3) pre-seed ~/.claude.json in `setup/symlink.sh` with just `{"theme":"dark-daltonized"}` if missing — undocumented, might get overwritten, but cheap. Option (1) is the cleanest until/unless Claude Code exposes theme via settings.json.
- **User-scoped plugin auto-install.** Per the Claude Code docs, declaring `extraKnownMarketplaces` + `enabledPlugins` in synced `settings.json` should cause Claude Code to auto-install declared/enabled plugins on first trust. Added the three GitHub-hosted marketplaces (`claude-plugins-official`, `anthropic-agent-skills`, `openai-codex`) — verify behavior on fresh Ona. If some plugins still need manual `/plugin install`, document which ones in README or add a setup hook.
